...
function makeWord(lettersObject) {
  const lettersArray = Object.entries(lettersObject).flatMap(
    ([letter, positions]) => positions.map((position) => [letter, position])
  );
  const word = lettersArray
    .sort((a, b) => a[1] - b[1])
    .map(([letter]) => letter)
    .join('');
